...
uint8_t XPT2046_Touch_Calibrate ( void )
{
    ...
    strType_XPT2046_Coordinate strCrossCoordinate [ 4 ], strScreenSample [ 4 ];
	  ...
	  strType_XPT2046_Calibration CalibrationFactor;
	    ...
	    usScreenHeight = LCD_Default_Max_Height;

	  #elif ( macXPT2046_Coordinate_GramScan == 2 ) || ( macXPT2046_Coordinate_GramScan == 3 )
	    usScreenWidth = LCD_Default_Max_Height;
	    usScreenHeight = LCD_Default_Max_Width;
		...
		strCrossCoordinate [ 0 ] .y = usScreenHeight >> 2;
		
		strCrossCoordinate [ 1 ] .x = strCrossCoordinate [ 0 ] .x;
		strCrossCoordinate [ 1 ] .y = ( usScreenHeight * 3 ) >> 2;
...
int backButton(void)				// Button handler for config screens (with back button)
{
		strType_XPT2046_Coordinate strDisplayCoordinate;
